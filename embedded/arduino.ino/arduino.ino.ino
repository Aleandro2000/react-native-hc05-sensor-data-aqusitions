...
void loop() {
  ...
  float heatIndex = -8.784695 + 1.61139411 * temp + 2.338549 * hum
                    - 0.14611605 * temp * hum - 0.01230809 * pow(temp, 2)
                    - 0.01642482 * pow(hum, 2) + 0.00221173 * pow(temp, 2) * hum
                    + 0.00072546 * temp * pow(hum, 2) - 0.00000358 * pow(temp, 2) * pow(hum, 2);
